...
class Feature_extractor(torch.nn.Module):
    ...
    def __init__(self,K,n_x,network_type):
        ...
        if self.network_type == 'mlp_mnist':
            ...
            layers.append(torch.nn.ReLU6())
            layers.append(torch.nn.Conv2d(32,128,5,2))
            self.f_shi_conv = torch.nn.Sequential(*layers)
            
            layers = []
            layers.append(torch.nn.Linear(128*5*5,128))
            layers.append(torch.nn.ReLU6())
            layers.append(torch.nn.Linear(128,self.K))
            self.f_shi_lin = torch.nn.Sequential(*layers)
...
class Dimension_encoder(torch.nn.Module):

    def __init__(self,K,n_x,network_type):
        super(Dimension_encoder,self).__init__()
        ...
        if self.network_type == 'mlp_mnist':
            ...
            layers.append(torch.nn.Linear(800,800))
            layers.append(torch.nn.ReLU())
            layers.append(torch.nn.Linear(800,2))
            layers.append(torch.nn.Softplus())
            self.pi_theta = torch.nn.Sequential(*layers)
        elif self.network_type == 'conv_net_fashion_mnist':
            layers = []
            layers.append(torch.nn.ReflectionPad2d(1))
            layers.append(torch.nn.Conv2d(1,32,5,2))
            layers.append(torch.nn.ReLU6())
            layers.append(torch.nn.Conv2d(32,128,5,2))
            self.pi_theta_conv = torch.nn.Sequential(*layers)
            ...
            layers.append(torch.nn.Softplus())
            self.pi_theta_lin = torch.nn.Sequential(*layers)


    def forward(self,x):
        if self.network_type == 'mlp_mnist':
            x = x.view(-1,self.n_x)
            ab = self.pi_theta(x) + 1e-7
            a = ab[:,0][:,None]
            b = ab[:,1][:,None]
            dim = np.arange(self.K,dtype=int)[None,:]
            fun1 = lambda r: sc.comb(self.K-1,r.item(),exact=False)
            combs = np.apply_along_axis(fun1,0,dim)
            ...
            mean_t_conv = self.pi_theta_conv(x) 
            mean_t_conv = mean_t_conv.view(-1,5*5*128)
            ab = self.pi_theta_lin(mean_t_conv) + 1e-7
            a = ab[:,0][:,None]
            b = ab[:,1][:,None]
            dim = np.arange(self.K,dtype=int)[None,:]
            fun1 = lambda r: sc.comb(self.K-1,r.item(),exact=False)
...
class nlIB_network(torch.nn.Module):
    ...
    def __init__(self,K,n_x,n_y,logvar_t=-1.0,train_logvar_t=False,network_type='mlp_mnist',method='nonlinear_IB',TEXT=None):
        ...
        if self.method == 'variational_IB':
            ...
            self.dim_encoder = Dimension_encoder(K,n_x,self.network_type)
    ...
    def encode(self,x,random=True):
        ...
        return t,gamma,pi,sigma_t,a,b,mean_t
    ...
    def forward(self,x):
        ...
        t,gamma,pi,_,_,_,_ = self.encode(x)

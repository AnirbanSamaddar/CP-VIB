...
class ConvexIB(torch.nn.Module):
    ...
    def __init__(self,n_x,n_y,problem_type,network_type,K,beta,dim_pen,a=1,b=1,logvar_t=-1.0,logvar_kde=-1.0,\
        ...
        self.network = nlIB_network(K,n_x,n_y,logvar_t,self.train_logvar_t,network_type,self.method,TEXT).to(dev)
    ...
    def get_IXT(self,t,mean_t,sigma_t,pi=1,pi_prior = 1):
        '''
        ...
        if self.method == 'nonlinear_IB':
            ...
            self.IXT = ((1-pi) * bin_entropy_est).mean(dim=1).div(math.log(2))
            self.IXT_1 = self.IXT
    ...
    def class_evaluate(self,logits_y,y,n_class):
        '''
        Evauluates the performance of the model for each class
        Parameters:
        - logits_y (Tensor) : deterministic transformation of the bottleneck variable
        - y (Tensor) : labels of the data
    ...
    def fit(self,trainset,validationset,n_epochs=200,learning_rate=0.0001,\
        ...
        if self.problem_type == 'classification' and (self.dataset_name == 'mnist' or self.dataset_name == 'fashion_mnist'):
            ...
            dim_prob_evolve = np.zeros((50*469,self.K))
        ...
        name_base = "K-" + str(self.K) + "-B-" + str(round(self.beta,3)).replace('.', '-') \
            + "_dim_pen_" + str(self.dim_pen).replace('.', '-') + "-Tr-" + str(bool(self.train_logvar_t)) + '-'
        ...
        for epoch in range(n_epochs+1):
            ...
            for idx_sgd_batch, sgd_batch in enumerate(bar(sgd_train_loader)):
                ...
                mi_train_t,_,mi_train_pi,mi_sigma_t,_,_,mi_train_mean_t = self.network.encode(mi_train_x,random=False)
                with torch.no_grad():
                    if epoch<=49:
                        dim_prob_evolve[(epoch*469+idx_sgd_batch),:] = mi_train_pi.mean(0).cpu()

                mi_train_IXT_1,mi_train_IXT_2 = self.get_IXT(mi_train_t,mi_train_mean_t,mi_sigma_t,mi_train_pi,pi_prior)
                if self.problem_type == 'classification':
            ...
            if epoch % eval_rate == 0:
                with torch.no_grad():
                    ...
                    for _, train_batch in enumerate(sgd_train_loader):
                        ...
                        train_t,_,train_pi,train_sigma_t,_,_,train_mean_t = self.network.encode(train_x,random=False)
                        tmp1,tmp2 =  self.get_IXT(train_t,train_mean_t,train_sigma_t,train_pi,pi_prior)
                    ...
                    validation_t,_,validation_pi,validation_sigma_t,_,_,validation_mean_t = self.network.encode(validation_x,random=False)
                    tmp1,tmp2 = self.get_IXT(validation_t,validation_mean_t,validation_sigma_t,validation_pi,pi_prior)
                ...
                with torch.no_grad():
                    ...
                    visualize_t,visualize_gamma,visualize_pi,visualize_sigma,visualize_a,visualize_b,visualize_t_mean = self.network.encode(visualize_x,random=False)
                    visualize_t_rand,visualize_gamma,visualize_pi,_,_,_,_ = self.network.encode(visualize_x,random=True)
